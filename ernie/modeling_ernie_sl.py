...
class ErnieModelForTokenClassification(ErnieModel):
    ...
    def forward(self, *args, **kwargs):
        ...
        labels = kwargs.pop('labels', None)
        label_sl = kwargs.pop('label_sl', None)
        ...
        if labels is not None:
            if len(labels.shape) != 2:
                labels = labels.squeeze()
            logits_softmax = fluid.layers.softmax(logits,axis=2)
            logits_sl = np.argmax(logits_softmax.numpy(), -1)
            
            logit_loss = P.to_tensor(0).astype('float32')
            if labels.shape[0] == 32:
                for l in range(labels.shape[0]):
                    if l % 2 == 1:
                        if l == 1 and l+2 < labels.shape[0]:
                            min_next = 100
                            min_next_index = 1000
                            count_ = 1
                            for i in labels[l+2][1:1+label_sl[l+2]]:
                                if int(i) < min_next and int(i) < 45:
                                    min_next = int(i)
                                    min_next_index = count_
                                count_ += 1
                                
                            if min_next >= 45:
                                min_next = 100
                            next_addr = min_next
                            next_addr_index = min_next_index
                            pre_addr = -1
                            pre_addr_index = -1
                        elif l == labels.shape[0] - 1 and l-2 > 0:
                            max_pre = -1
                            max_pre_index = -1
                            count_ = 1
                            for i in labels[l-2][1:1+label_sl[l-2]]:
                                if int(i) > max_pre and int(i) < 45:
                                    max_pre = int(i)
                                    max_pre_index = count_
                                count_ += 1
                            if max_pre < 0:
                                max_pre = -1
                            pre_addr = max_pre
                            pre_addr_index = max_pre_index
                            next_addr = 100
                            next_addr_index = 1000
                        else:
                            min_next = 100
                            min_next_index = 1000
                            count_ = 1
                        ...
                        if pre_addr == -1 or pre_addr_index == -1:
                            I_e1 = P.to_tensor(0).astype('float32')
                        else:
                            I_e1 = logits_softmax[l-2][pre_addr_index][pre_addr]
                        if next_addr == 100 or next_addr_index == 1000:
                            I_e3 = P.to_tensor(0).astype('float32')
                        else:
                            I_e3 = logits_softmax[l+2][next_addr_index][next_addr]
                        count_ = 1
                        average_count = 0
                        I_e2 = P.to_tensor(0).astype('float32')
                        for i in labels[l][1:1+label_sl[l]]:
                            if (i >= pre_addr and i <= next_addr) or (i >= next_addr and i <= pre_addr) :
                                I_e2 += logits_softmax[l][count_][int(i)]
                                average_count += 1
                            count_ += 1
                        if average_count >0:
                            I_e2 = I_e2/average_count
                        logit_loss += (1-min(1,1-max(0,I_e1+I_e3-1)+I_e2))
                        #print('logit_loss',logit_loss)
                        #print('ie1',I_e1)
                        #print('ie2',I_e2)
                        #print('ie3',I_e3)
                    
            logit_loss = logit_loss/labels.shape[0]
            #print('logit_loss',logit_loss)
