...
def evaluate_crf_case(model, model_crf, dataset):
    f_addr = open("./log/88/result.txt", "w+",encoding="utf-8")
    model.eval()
    final_label = []
    final_pred = []
    with P.no_grad():
        ...
        for step, (ids, sids, aligned_label, label, orig_pos
            ...
            for pos, lo, la, id in zip(orig_pos.numpy(),
                ...
                chunkf1.update((merged_preds, la, np.array(len(la))))
                result_str = ''
                for _ in merged_preds:
                    result_str += str(_) + ' '
                result_str += '\n'
                f_addr.write(result_str)                    
...
P1,R1,f1, each_f1,weight_f1 = evaluate_crf(model, model_crf,dev_ds)
c_P1,c_R1,c_f1, c_each_f1,c_weight_f1 = evaluate_crf_case(model, model_crf,dev_ds)
...
with open('./log/88/ernie-crf-sl-2.0.txt', 'w+', encoding='utf-8') as f1:
    f1.write(result_str)
if args.save_dir is not None:
    P.save(model.state_dict(),str( args.save_dir / 'ckpt.bin'))
